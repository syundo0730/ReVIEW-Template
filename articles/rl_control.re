...
とくに実ロボットを用いた強化学習の論文で言及されることが多い、最適制御の手法である、
...
== 離散システムの最適化問題
このシステムにおいて、以下の評価関数を最小にしたいと考えます。
//texequation{
...
V_i(x) = \min_u J_i(x, u)
//}
とすると、最適コストは部分最適コストで表現できるため、以下のように再帰的に表現できます。
//texequation{
\begin{aligned}
...
以上の離散システムにおいて、最適化を行う流れは以下のようになります。
...
式において@<m>$u$のみを設計変数として最適化していることから、このあと述べるDDP, iLQR, SLPもShooting法(Single Shooting)の一種ということになります。
...
以上の式, , , を用いて実際に最適制御入力を求めることができるのです。

== Differential Dynamic Programming(DDP)
以上をふまえて、離散システムの逐次最適化における手順2に、式を用いる方法がDDPです。
...
//}
です。
この離散システムを最小化する解は以下のRicatti方程式を解くことで得られることが知られています。
...
== iterative LQR(iLQR)
式を満たす最適制御入力は式から@<m>$f$の二階微分の項を除いたものになっています。
このため、DDPがニュートン法であったのに対してiLQRはガウスニュートン法を用いたものであるとも言えます。
iLQRはDDPのようにヘッシアンを計算する必要も逆行列を計算する必要もないため、DDPに比べて高速に処理できます。

== Sequential LQR(SLQ)
SLQはiLQRに対して、離散システムの逐次最適化における手順3で、式を用いたものです。
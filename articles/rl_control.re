...
\end{aligned}
//}
ここで、@<m>$L_t(x_t, u_t)$は最小化したいコスト関数です。

フィードバックの一つの方法として、状態フィードバックがあります。これは状態@<m>$x$を用いて制御入力@<m>$u$のみを決定する方法です。
線形のシステムと2次形式の評価関数においては、最適な線形状態フィードバックが解析的に求められますが、システムが非線形の場合や評価関数の形が2次形式でなければ工夫が必要になってきますし、望ましい性能が出ないことがあります。
線形ではない
フィードフォワードの方法というのは初期状態、初期入力から、状態方程式に基づいて将来の状態の系列と制御入力の系列を予測する方法です。
しかし初期状態から終端状態までを予測してもそれを実行している間にモデルの誤差などにより実際の動きは変わってくるため、逐次的に予測を行う、@<m>{モデル予測制御}が用いられることがあります。

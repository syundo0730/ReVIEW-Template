...
制御入力を求めるためのアプローチとしてはフィードバックを用いる方法とフィードフォワードで制御する方法があります。
フィードバックの一つの方法として、状態フィードバックがありますが、これは状態@<m>$x$を用いて制御入力@<m>$u$のみを決定する方法です。
線形のシステムと2次形式の評価関数においては、最適な状態フィードバックが求められますが、システムが非線形の場合や評価関数の形が2次形式でなければ工夫が必要になってきますし、望ましい性能が出ないことがあります。
フィードフォワードの方法というのは初期状態、初期入力から、状態方程式に基づいて将来の状態の系列と制御入力の系列を予測する方法です。
しかし初期状態から終端状態までを予測してもそれを実行している間にモデルの誤差などにより実際の動きは変わってくるため、逐次的に予測を行う、モデル予測制御が用いられることがあります。

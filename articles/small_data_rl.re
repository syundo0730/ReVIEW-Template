...
そして得られたガウス分布上でiLQRを使って最適な入力を決定します。
その上で、得られた状態と最適入力のペアの情報を使ってNeural Networkを学習します。
モデルをうまく使い、しかも最適な行動を決定する仕組みをNeural Networkに転写できているために、
画像inputから各関節への制御入力を生成するような、難易度の高い@<em>{End to End}方策の生成に成功しています。

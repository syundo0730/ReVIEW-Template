...
//image[premaidai_walking][教師として作られた歩行の動きで歩く様子]
...
平均報酬の遷移は@<img>{airl_progress}のようになりました。
//image[airl_progress][AIRLの学習における平均報酬の遷移][scale=0.7]
...
プリメイド AI 環境で学習を走らせてみます。

//cmd{
$ python run_ppo_bc.py
//}

平均報酬の遷移は@<img>{bc_ppo_progress}のようになりました。

//image[bc_ppo_progress][学習における平均報酬の遷移][scale=0.7]

歩くような脚の運びはありますが、バランスを取ることを学習しませんでした。

//image[bc_ppo_walk_premaid][学習結果][scale=0.6]

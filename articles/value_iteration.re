...
価値関数についてのベルマン方程式において、常に最適な方策を取るという前提を置けば、つぎの@<em>{最適ベルマン方程式}を定めることができます。
...
更に、取り得る方策が確率的でない、常に方策が決まっている定常方策を取るとすると、つぎのようになります。
...
== TD法
@<eq>{opt_const_v_belman_eq}の状態遷移確率が不明なのでつぎのように価値関数の推定値を更新していく手法がTD法です。
...
TD誤差として@<m>$\delta_t$をつぎのように定義することで、TD誤差を小さくする方向に更新するアルゴリズムとして捉えることもできます。
...
@<m>$s_{t+1}$において行う予定の行動@<m>$a_{t+1}$をもとに、つぎの更新則でQ値を更新します。
...
== Q-Learning
@<eq>{opt_const_q_belman_eq}の最適ベルマン方程式を解くために、つぎの更新則でQ値を更新するのがQ-Learningです。
...
近似関数を用いたQ-Learningのアルゴリズムはつぎのようになります。
...
//image[dqn_network][DQNのネットワークの概要]
DQNで用いられるニューラルネットワークの概要を@<img>{dqn_network}に示します。
...
そこでDQNを拡張した、DDQN(Double DQN)では、TD誤差をつぎの@<m>$y_t^{\text{DoubleDQN}}$と置き換えます。
//texequation{
y_t^{\text{DoubleDQN}}
...
DQNの後継の手法としては、近年の様々な知見をもとに、DQNを拡張していったRAINBOWという手法もあります。
現時点で、価値関数の推定の反復を用いた手法で採用する候補としてはRAINBOWが考えられるでしょう。
DQNとその亜種、後継の実装についてはネット上にも多くの情報があるので、本書ではあえて掲載しません。
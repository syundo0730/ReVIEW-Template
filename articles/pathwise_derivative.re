...
#@# もう一つの方法は@<eq>{likelihood_derivative}のように、期待値の外に微分をつける方法です。
...
行動と方策の更新を必ずしも同時に行う必要が無いため、方策オフの手法です。

== Stocastic Value Gradient(SVG)
ベルマン方程式において、終端状態を表現するためだけに時変の報酬@<m>$r^{(t)} = r(s^{(t)}, a^{(t)}, t)$を用いることを想定する
(一連の動作のうち最後の状態遷移で報酬$r^{(t)}$が得られるイメージ)。
このとき、連続空間におけるベルマン方程式は以下のようになります。
...
パラメタ@<m>$\theta$についての勾配@<m>$V_\theta$は式より、以下のようになります。
...
式の期待値の計算は、実際には確率変数@<m>$\eta, \xi$に基づくサンプリングをするモンテカルロ法を使って行います。
...
== SVG(@<m>$\infty$)
時間ステップ@<m>$T$について、式を用いて逆向きに計算し、@<m>$v^{(0)}_{\theta}$を実際に用いるのがSVG(@<m>$\infty$)です。
SVG(@<m>$\infty$)のアルゴリズムは、以下のようになります。
